    ...
    def seq_list():
        for h, seq in fasta_iter(fasta_file):
            if not split:
                ...
                onehalf = len(seq) // 2
                onethird = len(seq) // 3
                yield (h + '_a_1', seq[:onehalf])
                yield (h + '_a_2', seq[onehalf:])
                yield (h + '_b_1', seq[:1*onethird])
                yield (h + '_b_2', seq[1 * onethird:2 * onethird])
                yield (h + '_c_1', seq[:1 * onethird])
                yield (h + '_c_2', seq[2 * onethird:])
                yield (h + '_d_1', seq[1 * onethird:2 * onethird])
                yield (h + '_d_2', seq[2 * onethird:])
                yield (h + '_e_1', seq[:onehalf])
                yield (h + '_e_2', seq[:1 * onethird])
                yield (h + '_f_1', seq[:onehalf])
                yield (h + '_f_2', seq[1 * onethird:2 * onethird])
                yield (h + '_g_1', seq[:onehalf])
                yield (h + '_g_2', seq[2 * onethird:])
                yield (h + '_h_1', seq[onehalf:])
                yield (h + '_h_2', seq[:1 * onethird])
                yield (h + '_i_1', seq[onehalf:])
                yield (h + '_i_2', seq[1 * onethird:2 * onethird])
                yield (h + '_j_1', seq[onehalf:])
                yield (h + '_j_2', seq[2 * onethird:])
                yield (h + '_k_1', seq)
                yield (h + '_k_2', seq[:onehalf])
                yield (h + '_l_1', seq)
                yield (h + '_l_2', seq[onehalf:])
